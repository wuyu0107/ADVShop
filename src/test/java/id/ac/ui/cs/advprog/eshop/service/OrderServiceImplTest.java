...
public class OrderServiceImplTest {
    ...
    void setup() {
        ...
        orders.add(order1);
        Order order2 = new Order("7f9e15bb-4b15-42f4-aebc-c3af3856b078", products,
    ...
    void testUpdateStatus() {
        ...
        Order newOrder = new Order(order.getId(), order.getProducts(), order.getOrderTime(),
                order.getAuthor(), OrderStatus.SUCCESS.getStatus());
        doReturn(order).when(orderRepository).findById(order.getId());
        doReturn(newOrder).when(orderRepository).save(any(Order.class));

        Order result = orderService.updateStatus(order.getId(), OrderStatus.SUCCESS.getStatus());

        assertEquals(order.getId(), result.getId());
        assertEquals(OrderStatus.SUCCESS.getStatus(), result.getStatus());
